...
{
	// std::cout << "CALLBACK IMU QUATERNION" << std::endl;
	imu = *msg;
	...
	if(first_callback)	q = tf::Quaternion(0.0, 0.0, 0.0, 1.0);
		...
		q = q*tf::createQuaternionFromRPY(roll, pitch, yaw);
...
void callback_imu_matrix(const sensor_msgs::ImuConstPtr& msg)
{
	// std::cout << "CALLBACK IMU MATRIX" << std::endl;
	imu = *msg;
	...
	if(first_callback)	q = tf::Quaternion(0.0, 0.0, 0.0, 1.0);
		...
		if(R(0, 0)<-M_PI)	R(0, 0) += 2.0*M_PI;
		if(R(1, 0)>M_PI)	R(1, 0) -= 2.0*M_PI;
		if(R(1, 0)<-M_PI)	R(1, 0) += 2.0*M_PI;
		if(R(2, 0)>M_PI)	R(2, 0) -= 2.0*M_PI;
		if(R(2, 0)<-M_PI)	R(2, 0) += 2.0*M_PI;
	}
...
int main(int argc, char** argv)
{
	...
	// ros::Subscriber sub_imu = nh.subscribe("/imu/data", 10, callback_imu_matrix);
